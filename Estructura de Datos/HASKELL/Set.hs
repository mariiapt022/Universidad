...
delete x Empty = error "set vacio"
delete x (Node y (Node z s))
     | x == y =  s
     | x < y = Node y s 
...
union Empty (Node x y) = Node x y
union (Node x y) Empty = Node x y
union Empty Empty = Empty
union (Node x y) (Node z k) 
     | x == z = Node x (union y k)
     | x < z = Node x (Node z (union y k))
     | otherwise = Node z (Node (union y k) y)
