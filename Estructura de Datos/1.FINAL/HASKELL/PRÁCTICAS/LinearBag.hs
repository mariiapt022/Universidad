...
insert x (Node y c ys)
   | x==y = Node y (c+1) ys 
   | x<y = Node x 1 (Node y c ys)
   | x>y = Node y c (insert x ys)
...
delete x (Node y c ys)
   | x == y && c>1 = Node y (c-1) ys 
   | x == y && c==1 = ys 
   | x < y = Node y c ys 
   | x > y = Node y c (delete x ys)
...
union Empty xs = xs
union xs Empty = xs 
union (Node x c xs) (Node y k ys)
   | x == y = Node x (c+k) (union xs ys)
   | x < y = Node x c (union xs (Node y k ys))
   | x > y = Node y k (union (Node y k ys) xs)
...
intersection (Node x c xs) (Node y k ys)
   | x == y = Node x ( min k c) (intersection xs ys)
...
difference Empty xs = Empty
difference (Node x c xs) ys
   | oc > 0 && oc >= c = difference xs ys
   | oc > 0 && oc < c = Node x (c-oc) (difference xs ys)
   | otherwise = Node x c (difference xs ys)
      where oc = occurrences x ys 
