...
weights [] = D.empty
weights (x:xs) = D.insert x appearances (weight xs)
    where 
        appearances = case D.valueOf x (weight xs) of 
            Nothing -> 1
            Just c -> c+1
...
huffmanLeaves s
    | D.size weightsTree == 1 = error "La cadena solo tiene un caracter"
    | otherwise = D.foldKeysValues (\k v pq -> PQ.enqueue (WLeaf k v) pq) PQ.empty weightsTree
        where 
            weightsTree = weights s
