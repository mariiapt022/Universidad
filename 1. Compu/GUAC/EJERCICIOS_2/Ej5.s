...
pares:	ldr r0, [r1]		@ Bucle que va cargando en el registro r0 el valor a representar. r1 se incrementa tras su uso
		add r1,r1,#8
...
bucle2:	subS r3, r3, #1		@ El usar un bucle decreciente permite ahorrarnos una instrucción de comparación en cada iteración
		...
		sub r2,r2,#1
		cmp r2,r4		@ Bucle de control del número de dígitos que restan por mostrar
		bgt	pares			@ De nuevo, usamos un bucle decreciente para ahorrarnos una comparación
...
next:
		...
		add r1,r1,#4
impares:
	ldr r0, [r1]		@ Bucle que va cargando en el registro r0 el valor a representar. r1 se incrementa tras su uso
		add r1,r1,#8
		...
		ldr r3, =retardo	@ Cargamos de memoria el valor del retardo a esperar. 
							...
							@ Además, el retardo podría variarse dinámicamente durante la ejecución del programa
		ldr r3, [r3]		@ Inicializamos r3 con el valor del retardo. Observar que se ha reutilizado r3.
bucle3:	subS r3, r3, #1		@ El usar un bucle decreciente permite ahorrarnos una instrucción de comparación en cada iteración
		bNE bucle3			@ Salimos del bucle de retardo al llegar a 0
		sub r2, r2, #1		@ Bucle de control del número de dígitos que restan por mostrar
		cmp r2,r4
		bgt	impares	
